...
func _on_login_button_pressed() -> void:
	...
	_save_patient_data()
	_send_patient_to_python(current_patient['hospital_id']) 
	get_tree().change_scene_to_packed(MODE_SELECTION)  # scene changes immediately, no delay

func _send_patient_to_python(patient_id: String) -> void:
	"""Send hospital ID to Python via UDP port 9000 so it pre-fills the recording name."""
	...
	if command_socket.set_dest_address("127.0.0.1", 9000) == OK:
		...
		print("✅ Patient sent to Python: " + patient_id)
	else:
		print("❌ Failed to send patient to Python")

...
# Track current game name for recording (set by select_game / 3d_games via GlobalSignals)
var current_game_name: String = "UnknownGame"
...
# ── Play button pressed in timer popup ────────────────────────────────────────

func _on_timer_selector_play_pressed(time: int) -> void:
	timer_selector_instance.hide_panel()

	# ── Send start_recording to Python ────────────────────────────────────
	_send_recording_command("start_recording")

	# Forward to the game scene
...
func _on_countdown_timer_timeout() -> void:
	if countdown_active:
		...
		if countdown_time <= 0:
			...
			# ── Send stop_recording to Python when time is up ──────────────
			_send_recording_command("stop_recording")
...
func _send_recording_command(cmd_type: String) -> void:
	"""
	Send start_recording or stop_recording to Python on port 9000.
	game_name is read from GlobalSignals.current_game_name (set by select_game.gd).
	"""
	var game_name = GlobalSignals.get("current_game_name")
	...
	if sock.set_dest_address("127.0.0.1", 9000) == OK:
		...
		print("Recording command sent to Python: " + cmd_type + " | game: " + game_name)
	sock.close()


func stop_recording_now() -> void:
	"""Call this manually if game ends early (e.g. player quits mid-game)."""
	_send_recording_command("stop_recording")
	stop_countdown()
